...
/* STATE */
let state={sortCol:'nom',sortDir:1,filters:{consultant_id:'',type:'',before:'',objectif_id:''},objectifs_consultant_id:''};
/* CONSULTANTS LIST (col date supprimée) */
const consTBody=$$('#consultants-table tbody');
$$all('#consultants-table thead th[data-sort]').forEach(th=>th.onclick=()=>{state.sortCol=th.dataset.sort;state.sortDir*=-1;renderConsultants();});
function statusOf(c){
const p=store.params||DEFAULT_PARAMS, today=new Date();
...
const rows=[...store.consultants].sort((a,b)=> state.sortCol==='nom'
? state.sortDir * a.nom.localeCompare(b.nom)
: state.sortDir * (a.date_fin||'').localeCompare(b.date_fin||'')
);
$('consultants-count').textContent=rows.length;
consTBody.innerHTML='';
const options=['<option value="">👤 Tous les consultants</option>'];
rows.forEach(c=>{
const tr=document.createElement('tr'); tr.classList.add('clickable');
tr.innerHTML=`
<td>
<div class="row"><span class="dot ${statusOf(c)}"></span><b>${esc(c.nom)}</b></div>
<div class="sub">${esc(c.titre_mission||'—')}</div>
</td>
<td class="nowrap">
${c.url?`<a class="btn small" href="${esc(c.url)}" target="_blank" title="Fiche">🔗</a>`:''}
<button class="btn small" data-edit="${c.id}" title="Éditer">✏️</button>
<button class="btn small" data-goto-obj="${c.id}" title="Voir ses objectifs">🎯</button>
</td>`;
on(tr,'click',(e)=>{ if(e.target.closest('button, a')) return; setConsultantFilter(c.id); openTab('activite',true); });
tr.querySelector('[data-edit]').onclick=(e)=>{ e.stopPropagation(); openConsultantModal(c.id); };
tr.querySelector('[data-goto-obj]').onclick=(e)=>{ e.stopPropagation(); gotoConsultantObjectives(c.id); };
consTBody.appendChild(tr);
options.push(`<option value="${esc(c.id)}">${esc(c.nom)}</option>`);
});
if(selectConsultantMobile){
const current=state.filters.consultant_id||'';
selectConsultantMobile.innerHTML=options.join('');
if([...selectConsultantMobile.options].some(o=>o.value===current)) selectConsultantMobile.value=current;
else selectConsultantMobile.value='';
}
}
function setConsultantFilter(cid){ state.filters.consultant_id=cid; if(selectConsultantMobile) selectConsultantMobile.value=cid||''; renderActivities(); }
...
const inputBefore=$('filter-before');
const selectObjectif=$('filter-objectif');
const badgeCount=$('activities-count');
const selectConsultantMobile=$('filter-consultant-mobile');
const filterIcons={
consultant:$$('[data-filter-icon="consultant"]'),
type:$$('[data-filter-icon="type"]'),
objectif:$$('[data-filter-icon="objectif"]'),
before:$$('[data-filter-icon="before"]')
};
const expandedMobileActivities=new Set();
function updateFilterIcons(){
filterIcons.consultant?.classList.toggle('active',!!state.filters.consultant_id);
filterIcons.type?.classList.toggle('active',!!state.filters.type);
filterIcons.objectif?.classList.toggle('active',!!state.filters.objectif_id);
filterIcons.before?.classList.toggle('active',!!state.filters.before);
}
on(selectType,'change',e=>{state.filters.type=e.target.value; renderActivities();});
on(inputBefore,'change',e=>{state.filters.before=e.target.value; renderActivities();});
on(selectObjectif,'change',e=>{state.filters.objectif_id=e.target.value; renderActivities();});
$('btn-reset-filters').onclick=()=>{ state.filters={consultant_id:'',type:'',before:'',objectif_id:''}; selectType.value=''; inputBefore.value=''; selectObjectif.value=''; if(selectConsultantMobile) selectConsultantMobile.value=''; renderActivities(); };
$('btn-reset-filters-left').onclick=()=>{ state.filters.consultant_id=''; if(selectConsultantMobile) selectConsultantMobile.value=''; renderConsultants(); renderActivities(); };
on(selectConsultantMobile,'change',e=>{ setConsultantFilter(e.target.value||''); });
const TYPE_META={
...
const {consultant_id,type,before,objectif_id}=state.filters;
...
.filter(a=>!beforeDate || (parseDate(a.date_publication) && parseDate(a.date_publication) <= beforeDate))
...
<div class="text clamp-5">${descContent}</div>
...
textEl.classList.remove('clamp-5');
const fullHeight=textEl.scrollHeight;
textEl.classList.add('clamp-5');
...
selectObjectif.innerHTML = '<option value="">🎯 Tous les objectifs</option>' + store.objectifs.map(o=>`<option value="${o.id}">${esc(o.titre)}</option>`).join('');
if([...selectObjectif.options].some(o=>o.value===curr)) selectObjectif.value=curr;
}
function refreshAll(){ renderConsultants(); renderActivityFiltersOptions(); renderActivities(); renderObjectifs(); renderParams(); dashboard(); updateSyncPreview(); }
