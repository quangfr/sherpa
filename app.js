...
/* STATE */
let state={sortCol:'nom',sortDir:1,filters:{consultant_id:'',type:'',month:'',objectif_id:''},objectifs_consultant_id:''};
...
filterIcons.month?.classList.toggle('active',!!state.filters.month);
}
function refreshMonthOptions(){
  ...
  const options=['<option value="">📅 Toutes les dates</option>',...months.map(m=>`<option value="${m}">${formatMonthLabel(m)}</option>`)];
...
$('btn-reset-filters').onclick=()=>{ state.filters={consultant_id:'',type:'',month:'',objectif_id:''}; selectType.value=''; if(selectMonth) selectMonth.value=''; selectObjectif.value=''; if(selectConsultantMobile) selectConsultantMobile.value=''; renderActivities(); };
