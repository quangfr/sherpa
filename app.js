...
const filterSections={
  consultant:$$('[data-filter="consultant"]'),
  type:$$('[data-filter="type"]'),
  objectif:$$('[data-filter="objectif"]'),
  month:$$('[data-filter="month"]')
};
const expandedMobileActivities=new Set();
let monthOptionsCache='';
function updateFilterIcons(){
filterSections.consultant?.classList.toggle('active',!!state.filters.consultant_id);
filterSections.type?.classList.toggle('active',!!state.filters.type);
filterSections.objectif?.classList.toggle('active',!!state.filters.objectif_id);
filterSections.month?.classList.toggle('active',!!state.filters.month);
}
...
const btnFaDelete=$$('#dlg-activity .actions [value="del"]');
...
on(faDesc,'input',()=>autoSizeKeepMax(faDesc, ACT_DESC_MAX),{once:false});
 if(btnFaDelete){
  btnFaDelete.classList.toggle('hidden',!id);
  btnFaDelete.disabled=!id;
 }
dlgA.showModal();
}
